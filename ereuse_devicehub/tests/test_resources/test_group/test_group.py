...
class TestGroup(TestGroupBase):
    def test_group_full(self):
        ...
        self.post_and_check(self.PACKAGES, package1)
        device1, _ = self.get(self.DEVICES, item=device1_id)
        self.is_parent('package1', self.PACKAGES, device1_id, self.DEVICES)
        # Let's try to remove it
        package1_patch = {'@type': 'Package', 'children': {'devices': []}}
        self.patch_and_check('{}/{}'.format(self.PACKAGES, 'package1'), package1_patch)
        ...
        #    |
        # Package1
        lot1 = self.get_fixture(self.LOTS, 'input-lot')
        lot1['label'] = 'lot1'
        lot1['children']['packages'] = ['package1']
        self.post_and_check(self.LOTS, lot1)
        ...
        self.post_and_check(self.PACKAGES, package2)
        ...
        self.is_grandpa_or_above('lot1', self.LOTS, device1_id, self.PLACES)
        ...
        place1['label'] = 'place1'
        self.post_and_check(self.PLACES, place1)
        place2 = self.get_fixture(self.PLACES, 'place')
        place2['label'] = 'place2'
        self.post_and_check(self.PLACES, place2)
        # Let's make
        # Place1
        #   |
        # Place2
        place1_patch = {'@type': 'Place', 'children': {'places': ['place2']}}
        self.patch_and_check('{}/{}'.format(self.PLACES, 'place1'), place1_patch)
        ...
        self.patch_and_check('{}/{}'.format(self.PLACES, 'place2'), place2_patch)
        self.is_parent('place1', self.PLACES, 'place2', self.PLACES)
        ...
        lot1_patch = {'@type': 'InputLot', 'children': {'packages': []}}
        self.patch_and_check('{}/{}'.format(self.LOTS, 'lot1'), lot1_patch)
        ...
        lot1_patch = {'@type': 'InputLot', 'children': {'packages': ['package1']}}
        self.patch_and_check('{}/{}'.format(self.LOTS, 'lot1'), lot1_patch)
        self.is_parent('lot1', self.LOTS, 'package1', self.PACKAGES)
        self.is_grandpa_or_above('lot1', self.LOTS, device1_id, self.DEVICES)
        ...
        place3['label'] = 'place3'
        place3['children'] = {'places': ['place3']}
        self.post(self.PLACES, place3)
        ...
        self.patch_and_check('{}/{}'.format(self.PACKAGES, 'package2'), package2_patch)
        ...
        self.patch_and_check('{}/{}'.format(self.PLACES, 'place3'), place3_patch)
        ...
        self.is_not_grandpa_or_above('place3', self.PLACES, device2_id, self.DEVICES)
        # ...and of device1
        self.is_not_grandpa_or_above('place3', self.PLACES, 'package1', self.DEVICES)
        ...
        #              |
        #           Device2
        lot4 = self.get_fixture(self.LOTS, 'lot4')
        lot4['label'] = 'lot4'
        self.post_and_check(self.LOTS, lot4)
        lot3 = self.get_fixture(self.LOTS, 'lot')
        lot3['label'] = 'lot3'
        self.post_and_check(self.LOTS, lot3)
        lot5 = self.get_fixture(self.LOTS, 'lot')
        lot5['label'] = 'lot5'
        lot5['children'] = {'lots': ['lot3']}
        self.post_and_check(self.LOTS, lot5)
        ...
        self.post_and_check(self.PACKAGES, package3)
        lot3_patch = {'@type': 'Lot', 'children': {'packages': ['package3']}}
        self.patch_and_check('{}/{}'.format(self.LOTS, 'lot3'), lot3_patch)
        ...
        self.patch_and_check('{}/{}'.format(self.PACKAGES, 'package3'), package3_patch)
        ...
        # We won't be able to do this: a device is tied to the place of the package
        # Place1
        #   |  \
        #   |   \__
        # Place2 Place3      Lot4 Lot5
        ...
        _, status = self.patch('{}/{}'.format(self.PLACES, 'place3'), place3_patch)
        ...
        #              |         |
        #           Device2    Device3
        place3_patch['children']['devices'].remove(device3_id)
        place3_patch['children']['packages'] = ['package3']
        self.patch_and_check('{}/{}'.format(self.PACKAGES, 'package3'), place3_patch)
        ...
        lot5_patch = {'@type': 'Lot', 'devices': [device4_id]}
        self.patch_and_check('{}/{}'.format(self.LOTS, 'lot5'), lot5_patch)
        self.is_parent('lot5', self.LOTS, 'device4', self.DEVICES)  # Lot 5 is parent
        self.is_grandpa_or_above('lot5', self.LOTS, 'device4', self.DEVICES)  # And keeps being a grandparent/above
        self.is_grandpa_or_above('lot4', self.LOTS, device4_id, self.DEVICES)
        ...
        #              |         |         |
        #           Device2    Device3  Device4
        place4 = self.get_fixture(self.PLACES, 'place4')
        place4['label'] = 'place4'
        place4['children'] = {'devices': [device4_id]}
        self.post_and_check(self.PLACES, place4)
        self.is_parent('place4', self.PLACES, device4_id, self.DEVICES)
        # The rest is the same for device4
        self.is_parent('lot5', self.LOTS, 'device4', self.DEVICES)
        self.is_grandpa_or_above('lot5', self.LOTS, 'device4', self.DEVICES)
        self.is_grandpa_or_above('lot4', self.LOTS, device4_id, self.DEVICES)
