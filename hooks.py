def event_hooks(app):
    ...
    from app.resources.device.hooks import generate_etag, get_icon, get_icon_resource, autoincrement, post_benchmark, \
    ...
    from app.resources.event.snapshot.hooks import on_insert_snapshot, save_request, materialize_test_hard_drives, \
    ...
    from app.resources.event.hooks import get_place, materialize_components, materialize_parent, set_place
    ...
    from app.resources.event.add.hooks import add_components
    app.on_inserted_add += add_components

    from app.resources.event.register.hooks import post_devices
    app.on_insert_register += post_devices

    from app.resources.event.remove.hooks import remove_components
    app.on_inserted_remove += remove_components

    from app.resources.event.allocate.hooks import materialize_actual_owners_add, avoid_repeating_allocations, set_organization
    ...
    from app.resources.event.deallocate.hooks import materialize_actual_owners_remove, set_organization
    ...
    if app.config.get('LOGGER', True):
        from app.resources.event.logger.hooks import get_info_from_hook
        app.on_inserted += get_info_from_hook

    from app.resources.account.hooks import add_token, hash_password, set_default_database_if_empty
    app.on_insert_accounts += add_token
    app.on_insert_accounts += hash_password
    app.on_insert_accounts += set_default_database_if_empty

    from app.resources.account.hooks import set_byUser, add_or_get_inactive_account, set_byOrganization
    ...
    from app.resources.event.receive.hooks import transfer_property, set_organization
    app.on_insert_receive += transfer_property
    app.on_insert_receive += set_organization

    from app.resources.place.hooks import set_place_in_devices, update_place_in_devices, unset_place_in_devices, update_place_in_devices_if_places, avoid_deleting_if_has_event
