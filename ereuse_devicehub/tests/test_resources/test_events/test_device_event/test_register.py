...
class TestRegister(TestEvent, TestGroupBase):
    ...
    def test_register_placeholder(self):
        ...
        event = self.post_201(self.REGISTER_URL, placeholder)
        event = self.get_200(self.EVENTS, item=event['_id'])
        device = self.get_200(self.DEVICES, '', event['device'])
        ...
        place = self.post_201(self.PLACES, copy.deepcopy(place))
        ...
        full_snapshot = self.post_201('{}/{}'.format(self.DEVICE_EVENT, self.SNAPSHOT), full_snapshot)
    ...
    def test_placeholder_endpoint(self):
        ...
        response = self.post_201('events/devices_register/placeholders?quantity=3', {})
