...
class TestGroup(TestGroupBase):
    def test_add(self):
        ...
        package = self.get_and_check('packages', item=_id)
        assert_that(package).has_label('package1')

        pallet = self.get_fixture(self.GROUPS, 'pallet')
        _id = self.post_and_check(self.PALLETS, pallet)['_id']
        db_pallet = self.get_and_check(self.PALLETS, item=_id)
    ...
    def test_group_full(self):
        ...
        not_ancestors = ((lot1_id, self.LOTS), (place2_id, self.PLACES), (place3_id, self.PLACES), (place1_id, self.PLACES))
        ...
        self.is_grandpa_or_above(lot5_id, self.LOTS, device4_id, self.DEVICES)  # Lot 5 is already a grandparent or above
    ...
    def test_prohibited_actions(self):
        ...
        place['label'] = 'place1'
        place_id = self.post_and_check(self.PLACES, place)['_id']
